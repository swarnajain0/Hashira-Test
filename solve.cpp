...
// ==================== Tiny BigInt (base 1e6) ====================
struct BigInt {
    static const uint32_t BASE = 1000000U; // 1e6
    ...
    static BigInt addAbs(const BigInt& a, const BigInt& b) {
        BigInt c; c.sign = a.sign ? a.sign : (b.sign ? b.sign : 1);
    ...
    static BigInt subAbs(const BigInt& a, const BigInt& b) { // assumes |a|>=|b|
        BigInt c; c.sign = a.sign;
        long long carry = 0;
        c.d.resize(a.d.size());
        for (size_t i=0;i<a.d.size();i++) {
            long long ai = a.d[i];
            long long bi = (i<b.d.size()?b.d[i]:0);
    ...
    friend BigInt operator+(const BigInt& a, const BigInt& b) {
        ...
        if (a.sign == b.sign) return addAbs(a,b);
        int c = cmpAbs(a,b);
        if (c==0) return BigInt(0);
        if (c>0) { BigInt r=subAbs(a,b); r.sign=a.sign; return r; }
        BigInt r=subAbs(b,a); r.sign=b.sign; return r;
    }
    ...
    friend BigInt mulSmall(const BigInt& a, uint64_t m) {
        ...
        while (carry) {
            c.d.push_back((uint32_t)(carry%BASE));
            carry /= BASE;
        }
        c.trim();
        return c;
    ...
    friend BigInt operator*(const BigInt& a, const BigInt& b) {
        ...
        for (size_t i=0;i<a.d.size();i++) {
            unsigned long long carry=0ULL;
            unsigned long long ai=a.d[i];
            for (size_t j=0;j<b.d.size() || carry; j++) {
                unsigned long long cur = c.d[i+j] +
                    (unsigned long long)ai * (j<b.d.size()?b.d[j]:0ULL) + carry;
    ...
    uint32_t divSmall(uint32_t v) {
        if (v==0) throw std::runtime_error("div by zero");
        if (isZero()) return 0;
        unsigned long long rem=0ULL;
        for (int i=(int)d.size()-1;i>=0;i--) {
            unsigned long long cur = d[i] + rem*BASE;
            uint32_t q = (uint32_t)(cur / v);
            uint32_t r = (uint32_t)(cur % v);
            d[i]=q; rem=r;
        }
        trim();
        if (sign==0) rem=0;
    ...
    uint32_t modSmall(uint32_t v) const {
        if (v==0) throw std::runtime_error("mod by zero");
        if (isZero()) return 0;
        unsigned long long rem=0ULL;
        for (int i=(int)d.size()-1;i>=0;i--) {
            unsigned long long cur = d[i] + rem*BASE;
    ...
    std::string toString() const {
        ...
        while (!tmp.isZero()) {
            parts.push_back(tmp.divSmall(1000000U));
        }
        std::string s = (sign<0?"-":"");
        for (int i=(int)parts.size()-1;i>=0;i--) {
            std::string chunk = std::to_string(parts[i]);
            if (i!=(int)parts.size()-1) s += std::string(6 - (int)chunk.size(),'0');
            s += chunk;
        }
        return s;
    }
...
// ==================== Rational (BigInt / int64) ====================
struct Rational {
    BigInt num;
    long long den;

    Rational(long long v=0): num(v), den(1) {}
    Rational(const BigInt& n, long long d): num(n), den(d) { normalize(); }

    void normalize() {
        if (num.isZero()) { den=1; return; }
        if (den<0) { den = -den; num.sign = -num.sign; }
    }

    friend Rational operator+(const Rational& A, const Rational& B) {
        if (A.num.isZero()) return B;
        if (B.num.isZero()) return A;
        long long ad=A.den, bd=B.den;
        long long g1 = igcd(ad, bd);
        long long denL = (ad/g1)*bd;
        BigInt t1 = mulSmall(A.num, (uint64_t)(bd/g1));
        BigInt t2 = mulSmall(B.num, (uint64_t)(ad/g1));
        BigInt n = t1 + t2;
        return Rational(n, denL);
    }
    friend Rational operator-(const Rational& A, const Rational& B) {
        Rational negB = B; if (!negB.num.isZero()) negB.num.sign = -negB.num.sign;
        return A + negB;
    }
    friend Rational operator*(const Rational& A, const Rational& B) {
        if (A.num.isZero() || B.num.isZero()) return Rational(0);
        BigInt n = A.num * B.num;
        long long d = A.den * B.den;
        return Rational(n, d);
    }
    friend Rational mulInt(const Rational& A, long long k) {
        if (k==0 || A.num.isZero()) return Rational(0);
        BigInt n = mulSmall(A.num, (uint64_t)(k<0?-k:k));
        if (k<0 && !n.isZero()) n.sign = -n.sign;
        return Rational(n, A.den);
    }
};

// ==================== Parse base-b to BigInt ====================
...
static BigInt parseInBase(const std::string& s, int base) {
    BigInt val(0);
    for (char c: s) {
        if (std::isspace((unsigned char)c) || c=='_') continue;
        int d = digitVal(c);
        val = mulSmall(val, (uint64_t)base);
        BigInt addv(d);
        val = val + addv;
    }
    return val;
...
static std::vector<Rational> lagrange_coeffs(const std::vector<long long>& xs, const std::vector<BigInt>& ys) {
    int m = (int)xs.size() - 1;
    std::vector<Rational> coeffs(m+1, Rational(0));
    for (int i=0;i<=m;i++) {
        long long xi = xs[i];
        std::vector<BigInt> basis(1); basis[0]=BigInt(1);
        long long denom = 1;
        for (int j=0;j<=m;j++) if (j!=i) {
            long long xj = xs[j];
            std::vector<BigInt> nxt(basis.size()+1, BigInt(0));
            for (size_t k=0;k<basis.size();k++) {
                BigInt term = mulSmall(basis[k], (uint64_t)(xj<0?-xj:xj));
                if (xj > 0 && !term.isZero()) term.sign = -term.sign;
                nxt[k] = nxt[k] + term;
                nxt[k+1] = nxt[k+1] + basis[k];
            }
            denom *= (xi - xj);
            basis.swap(nxt);
        }
        for (size_t k=0;k<basis.size();k++) {
            BigInt num = basis[k] * ys[i];
            Rational term(num, denom);
            coeffs[k] = coeffs[k] + term;
        }
    }
    return coeffs;
}
...
int main() {
    ...
    if (n<=0 || k<=0) { cerr<<"Failed to parse n/k\n"; return 1; }
    ...
    if ((int)xs.size() < k) { cerr<<"Only "<<xs.size()<<" points, need k="<<k<<"\n"; return 1; }
    ...
    for (size_t i = 0; i < xs.size(); ++i) {
        auto y = parseInBase(vals[i], bases[i]);
        cout << "[DECODED] x=" << xs[i]
             << " y=" << y.toString() << "\n";
    }
    ...
    for (size_t i=0;i<xs.size();i++) {
        BigInt y = parseInBase(vals[i], bases[i]);
    ...
    for (int i=0;i<=mdeg;i++) {
        if (C[i].den==1) {
            cout << "c["<<i<<"] = " << C[i].num.toString() << "\n";
        } else {
            cout << "c["<<i<<"] = " << C[i].num.toString() << "/" << C[i].den << "\n";
        }
    }
    return 0;
}
